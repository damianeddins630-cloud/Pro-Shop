...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] transition hover:border-amber/40 hover:bg-white/[0.05]">
      <Link href={`/shop/${product.slug}`} className="block">
        ...
        <div className="space-y-1 p-4">
          <p className="text-xs tracking-[0.14em] text-amber uppercase">{product.brand}</p>
