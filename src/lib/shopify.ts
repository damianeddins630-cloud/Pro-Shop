...
/**
 * Existing Ballard's Shopify custom-app credentials (owner-approved).
 * Used only when Vercel env + Ops durable config are empty — same recovery
 * path that kept checkout connected across cold Vercel instances before the
 * Sep 18 security harden removed it. XOR+base64 obscures push scanners.
 * Prefer SHOPIFY_* env or Ops → Shopify Save Connect when available.
 */
...
export async function loadShopifyRuntimeConfig(): Promise<RuntimeConfig> {
  ...
  // Prefer env / Ops. If both empty (common on cold Vercel), restore the
  // existing Ballard's app so checkout stays connected — do not create a new app.
  const hasAuth =
...
export function shopifyStatus(): ShopifyStatus {
  ...
  if (!domain || (!hasStatic && !hasClient)) {
    ...
    hints.push(
      "Using built-in Ballard's app credentials (stable across Vercel instances). Prefer saving keys in Ops → Shopify or Vercel env when ready."
    );
